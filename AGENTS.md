...
- 启动：`./vibe-server`，监听 `:8080`。环境变量 `PORT`、`VIBE_PLATFORM`。
...
- 默认概念版(lite)：`main.go` 调 `api.SetUseLite(true)`；`VIBE_PLATFORM=std` 切标准版。两版共用算法，**仅盐值与 appid/clientver 不同**，相关常量集中在 request.go 顶部。
...
- **登录后用户类接口的坑（get_my_info / get_all_list，踩过重雷）**：
  - **平台匹配（关键）**：这两接口必须用**与登录 token 相同的平台**签名+RSA 公钥。本项目全局是概念版(lite)，账号/登录也走概念版：
    - `GetUserDetail`（get_my_info）：**必须用概念版 RSA 公钥**（`RawRSAEncrypt(json, true)`）+ lite 签名（**不要 StdPlatform**）。用标准版公钥/签名 → 20018/20006。
    - `GetUserPlaylist`（get_all_list）：用 lite 签名（**不要 StdPlatform**），概念版 token 返回歌单正常；用标准版签名 → 20017。
  ...
  - `RequestOptions` 新增 `StdPlatform`（强制标准版）、`MIDSet`/`DFIDSet`（空值不自动 fallback 到全局 GetMID/GetDFID）。
...
- **明确不做**：云盘、MV/视频、青少年频道、长音频、场景/主题、音乐人/IP。
