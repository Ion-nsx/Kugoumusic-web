...
- [x] 歌词搜索 `search_lyric` → `/api/search/lyric`（已迁移 lyrics.kugou.com/v1/search，原 mobilecdn 404）
...
## 云盘
- [x] 云盘列表 `user_cloud` → `/api/cloud/list`（mcloudservice + AES+RSA 加密）
- [x] 云盘删除 `user_cloud_del` → `/api/cloud/delete`
- [x] 云盘播放 `user_cloud_url` → `/api/cloud/url`（bsstrackercdngz + signCloudKey）
- [x] 前端 Cloud.vue：列表/播放/删除/分页/歌名解析
- [ ] 云盘上传 `user_cloud_upload`（后续补）
- [ ] 云盘匹配 `user_cloud_match`
